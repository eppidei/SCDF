...
mDNSlocal int SetupSocket(struct sockaddr *intfAddr, mDNSIPPort port, int interfaceIndex, int *sktPtr)
{
    ...
    if (intfAddr->sa_family == AF_INET)
    {
        ...
        if (err == 0)
        {
            ...
            bindAddr.sin_addr.s_addr = INADDR_ANY;//inet_addr("224.0.0.251");//; // Want to receive multicasts AND unicasts on this socket
...
mDNSexport mStatus mDNSPlatformInit(mDNS *const m)
{
    ...
    if (err == mStatus_NoError) err = SetupInterfaceList(m); /*LEO TRYING to avoid interface scanning */
//  sa1.sin_family = AF_INET;
// sa1.sin_addr.s_addr = inet_addr("192.168.1.52");
//  sa2.sin_family = AF_INET;
//  sa2.sin_addr.s_addr = inet_addr("255.255.255.0");
//
//  if (err == mStatus_NoError)  err = SetupOneInterface(m, (struct sockaddr*)&sa1, (struct sockaddr*)&sa2, "wlan0", 0);
...
#if !defined(LEO_ELIMINA_STUBS_DUPLICATE)
...
#endif // !defined(LEO_ELIMINA_STUBS_DUPLICATE)
// DNSSEC stub functions
