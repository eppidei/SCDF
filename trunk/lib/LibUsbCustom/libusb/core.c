...
// NB: the original libusb_open is actually calling libusb_open_with_fd, 
// that's the original libusb_open with the possibility to indicate an 
// already opened fd. if you pass fd=-1, you'll get the usual behaviour.

int API_EXPORTED libusb_open(libusb_device *dev, libusb_device_handle **handle)
{
	return libusb_open_with_fd(dev,handle,-1);
}


int API_EXPORTED libusb_open_with_fd(libusb_device *dev,
	libusb_device_handle **handle, int fd) // fd added for android;
{
	...
	if (fd<0)
		...
		r = usbi_backend->open_with_fd(_handle,fd); // for android
...
{
	return libusb_open_device_with_vid_pid_and_fd(ctx,vendor_id,product_id,-1);
}

DEFAULT_VISIBILITY
libusb_device_handle * LIBUSB_CALL libusb_open_device_with_vid_pid_and_fd(
	libusb_context *ctx, uint16_t vendor_id, uint16_t product_id, int fd)
{
	...
	if (found) {
		r = libusb_open_with_fd(found, &handle, fd) ; // android modification for fd
							      // NB: libusb_open_with_fd will deal with -1 case
		if (r < 0)
			handle = NULL;
	}
