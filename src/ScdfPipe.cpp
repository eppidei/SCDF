...
s_int32 ScdfPipe::SendMessage(PipeMessage msg)
{
    _ASSERT(!invalid);
    if (invalid) return 0;
    s_int32 bytesWritten=0;
    s_int32 ret=Write((const s_char *)msg, SizeofPipeMessage, &bytesWritten);
    if(!ret)
    {
        Logging::Instance()->log(_T("Error writing to pipe"));
        return 0;
    }
    _ASSERT(bytesWritten==SizeofPipeMessage);
    return 1;
}

PipeMessage ScdfPipe::ReadMessage()
{
    _ASSERT(!invalid);
    if (invalid) return NULL;
    s_int32 numBytesRead=0;
    PipeMessage msg;
    if (Read((s_char *)msg, SizeofPipeMessage, &numBytesRead))
    {
    ...
    _ASSERT(numBytesRead==SizeofPipeMessage);
    return msg;
}
